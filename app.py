...
BMI_plot = st.sidebar.checkbox("Plot BMI")
age_plot = st.sidebar.checkbox("Plot age")
pregnancies_plot = st.sidebar.checkbox("Plot pregnancies")
glucose_plot = st.sidebar.checkbox("Plot glucose")
skinthickness_plot = st.sidebar.checkbox("Plot skinthickness")
bloodpressure_plot = st.sidebar.checkbox("Plot bloodpressure")
insulin_plot = st.sidebar.checkbox("Plot insulin")
dpf_plot = st.sidebar.checkbox("Plot dpf")
    
if st.checkbox('Show plot'):
    chart_data = pd.read_csv('diabetes.csv')
    feat_cols = []
    if BMI_plot:
        feat_cols.append('BMI')
    if age_plot:
        feat_cols.append('Age')
    if pregnancies_plot:
        feat_cols.append('Pregnancies')
    if glucose_plot:
        feat_cols.append('Glucose')
    if skinthickness_plot:
        feat_cols.append('SkinThickness')
    if bloodpressure_plot:
        feat_cols.append('BloodPressure')
    if insulin_plot:
        feat_cols.append('Insulin')
    if dpf_plot:
        feat_cols.append('DiabetesPedigreeFunction')
    st.line_chart(chart_data[feat_cols])
